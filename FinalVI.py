...
def main(page: ft.Page):
    ...
    def apply_search_filter(book_list: List[Book]) -> List[Book]:
        ...
        for b in book_list:
            ...
            ok_genre = True
            if gf:
                ok_genre = gf in (b.tags or "").lower()

            if ok_text and ok_genre:
                result.append(b)
    ...
    def on_file_picked(e: ft.FilePickerResultEvent):
        ...
        # Verificar duplicado por ruta de archivo
        canonical = str(path.resolve())
        for b in books:
            if b.file_path and str(b.file_path.resolve()) == canonical:
                page.snack_bar = ft.SnackBar(
                    ft.Text("Ese libro ya está en la biblioteca.")
                )
    ...
    def calc_settings_width() -> float:
        ...
        max_width = 600
        # algunas versiones de Flet no tienen window_width, así que usamos width
    ...
    # ---------- Funciones de refresco / historial / filtro género ----------
    ...
    def clear_history(e):
        """Marca todos los libros como no leídos y resetea la última lectura."""
        cur = conn.cursor()
        cur.execute("UPDATE books SET is_read = 0, current_page = 0")
        cur.execute("UPDATE last_read SET book_id = NULL, page = 0 WHERE id = 1")
        conn.commit()
        ...
        page.snack_bar = ft.SnackBar(ft.Text("Historial de lectura borrado."))
        page.snack_bar.open = True
        refresh_current_view()
        page.update()

    def open_filter_dialog(e):
        nonlocal genre_filter

        field = ft.TextField(
            label="Filtrar por género (usa las etiquetas/tags)",
            hint_text="Ej: fantasía, terror, romance…",
            value=genre_filter,
            width=400,
        )
        ...
        def apply_filter(ev):
            nonlocal genre_filter
            genre_filter = (field.value or "").strip()
            close_dialog(ev)
            refresh_current_view()

        dialog = ft.AlertDialog(
            title=ft.Text("Filtrar por género"),
            content=field,
            actions=[
                ft.TextButton("Cancelar", on_click=close_dialog),
                ft.ElevatedButton("Aplicar", on_click=apply_filter),
            ],
    ...
    def clear_genre_filter(e):
        nonlocal genre_filter
        genre_filter = ""
        page.snack_bar = ft.SnackBar(ft.Text("Filtro de género eliminado."))
        page.snack_bar.open = True
        refresh_current_view()
        page.update()

    # ---------- Barra de búsqueda con menú hamburguesa ----------
    def build_search_bar():
        menu_button = ft.PopupMenuButton(
            ...
            items=[
                ...
                ft.PopupMenuItem(
                    text="Quitar filtro de género",
                    on_click=clear_genre_filter,
                ),
            ],
        )
    ...
    def on_search_change(e):
        ...
        if idx == 0:  # Historial
            ...
            # En Explorar: search_bar, título, botón, "Biblioteca", fila de libros
