...
def main(page: ft.Page):
    ...
    content=ft.Container(),  # <--- este es el fix
    expand=True,
    transition=ft.AnimatedSwitcherTransition.SCALE,  # o FADE
    duration=350,
    ...
    def build_books_row(book_list: List[Book]):
        ...
        return ft.Row(
            controls=[
                BookCard(
                    b,
                    on_click=lambda e, bk=b: open_reader(bk),
                )
                for b in book_list
            ],
            spacing=10,
    ...
    def show_explore_view():
        ...
        current_view.controls.append(build_books_row(books))
